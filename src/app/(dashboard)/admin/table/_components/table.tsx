...
import DropdownAction from "@/components/common/drowpdown-action";
...
import { HEADER_TABLE_TABLES } from "@/constants/table-constant";
...
export default function TableManagement() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (currentSearch) {
        query.or(
          `name.ilike.%${currentSearch}%,capacity.ilike.%${currentSearch}%,capacity.ilike.%${currentSearch}%`
        );
      }
  ...
  const filteredData = useMemo(() => {
    return (tables?.data || []).map((table: Table, index) => {
      return [
        ...
        //     tabl.is_available ? "bg-green-600" : "bg-red-500"
        //   )}
        // >
